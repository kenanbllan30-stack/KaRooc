...
bot.on('message', (msg) => {
  ...
  if (text === '/start') {
    ...
    bot.sendMessage(chatId, `💳 **شحن عبر شام كاش**\n\nمعرف الحساب المحول إليه:\n\`B38774eaac6e58f5f1e26d79a5f2522a\`\n\nالرجاء تحويل المبلغ المطلوب إلى الحساب أعلاه، ثم **أرسل هنا في الصندوق رقم الحوالة أو صورة الإيصال مع الـ ID الخاص بك (${chatId})** لتتم مراجعتها وشحن رصيدك فوراً ⏳`, { parse_mode: 'Markdown' });
  }
  else if (text === '📱 سيرياتيل كاش (Syriatel Cash)') {
    userStates[chatId] = { method: 'Syriatel Cash' };
    bot.sendMessage(chatId, `📱 **شحن عبر سيرياتيل كاش**\n\nرقم الحساب المحول إليه:\n\`57480992\`\n\nالرجاء تحويل المبلغ إلى الرقم أعلاه، ثم **أرسل هنا رقم الحوالة أو تفاصيل التحويل مع الـ ID الخاص بك (${chatId})** لتأكيد الشحن ⏳`, { parse_mode: 'Markdown' });
  }
    ...
    if (userStates[chatId]) {
      ...
      bot.sendMessage(chatId, `✅ **تم استلام طلب الشحن بنجاح!**\nطريقة الدفع: ${paymentMethod}\nالتفاصيل المرسلة: ${text}\n\nجاري التحقق من الحوالة من قبل الإدارة وسيتم إضافتها لرصيدك خلال دقائق 🚀`);
      delete userStates[chatId];
    } else {
...
console.log('Bot with Real Payment Accounts is running...');
